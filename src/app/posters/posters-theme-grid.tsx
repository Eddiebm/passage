...
type PosterFilter = 'all' | 'africa-extended' | VisualThemeGroup

function filterThemes(filter: PosterFilter) {
...
export function PostersThemeGrid() {
  ...
  function setPosterParams(next: { filter?: PosterFilter; page?: number }) {
    const params = new URLSearchParams(searchParams.toString())
    const nextFilter = next.filter ?? filter
    const nextPage = next.page ?? 1
    params.set('filter', nextFilter)
    params.set('page', String(nextPage))
    router.push(`/posters?${params.toString()}`, { scroll: false })
  }
    ...
    <div className="mt-8 space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
        <div className="flex flex-wrap gap-2">
          <FilterChip active={filter === 'all'} onClick={() => setPosterParams({ filter: 'all', page: 1 })}>
          ...
          {VISUAL_THEME_GROUPS.map((g) => (
            <FilterChip
              key={g.id}
              active={filter === g.id}
              onClick={() => setPosterParams({ filter: g.id, page: 1 })}
            >
              {g.label}
            </FilterChip>
          ))}
          <FilterChip
            ...
            onClick={() => setPosterParams({ filter: 'africa-extended', page: 1 })}
          >
            Across Africa ({AFRICA_EXTENDED_THEME_COUNT})
          </FilterChip>
        </div>
        <label className="block w-full sm:max-w-xs">
          <span className="sr-only">Search burial posters</span>
          <input
            type="search"
            value={query}
      ...
      {!query && totalPages > 1 && (
        <PaginationBar page={page} totalPages={totalPages} onPage={(p) => setPosterParams({ page: p })} />
      )}
            ...
            >
              ...
              <div className="border-t border-[#3D2B1F]/10 px-4 py-3">
                ...
                <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-sm font-medium">
                  ...
                  <Link
                    href={completeShowcaseCreateHref(themeId)}
                    className="text-[#1A1A1A]/70 underline-offset-2 hover:underline"
                  >
                    Use on memorial
                  </Link>
                </div>
      ...
      {visibleThemes.length === 0 && (
        <p className="text-sm text-[#1A1A1A]/60">No burial posters match your search on this page.</p>
      )}
